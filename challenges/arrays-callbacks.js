...
function getNames(arr){
  const lowCaseAnimalNames = arr.map((item) => {
    return item.animal_name.toLowerCase();
  })
  return lowCaseAnimalNames;
}
console.log(getNames(zooAnimals));
...
function lowPopulation(arr){
  const lowPopulationAnimals = arr.filter((item) => {
...
function totalUsPopulation(arr) {
  const populationTotal = arr.reduce((total, item) => {
    return total += item.population;
  }, 0)
  return populationTotal;
}
console.log(totalUsPopulation(zooAnimals));
